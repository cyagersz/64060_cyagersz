...
Normalized the data
```{r}
cereal_numeric_norm<-sapply(cereal_numeric,scale)
...
I Put each cereal into a cluster, I choose 6 clusters because I felt it split the cereals into around the same number  per cluster and after 6 it just lowers some clusters and not evenly.
```{r}
# Cut into k clusters
...
partition_A <- cereal_numeric.df.norm[train_index, ]
partition_B <- cereal_numeric.df.norm[-train_index, ]
```

```{r}
# Cluster Partition A
set.seed(123)
kA <- kmeans(partition_A, centers = 6)
kA
kA$centers 

# Cluster Partition B based on Partition A centroids
kB <- kmeans(partition_B, centers = kA$centers)
kB
kB$centers


fviz_cluster(kB, data = partition_B) +
  ggtitle("Partition B Clusters Based on Partition A Centroids")

k_full <- kmeans(cereal_clusters, centers = 6)
k_full
k_full$centers

fviz_cluster(k_full,cereal_clusters) +
  ggtitle("Full Data")

# Add cluster assignments to Partition B
partitionB_clusters <- data.frame(partition_B, Cluster = kB$cluster)

# List cereals in each cluster for Partition B
split(rownames(partitionB_clusters), partitionB_clusters$Cluster)

# Add cluster assignments to full dataset
full_clusters <- data.frame(cereal_numeric.df.norm, Cluster = k_full$cluster)

# List cereals in each cluster for full dataset
split(rownames(full_clusters), full_clusters$Cluster)

```
Partition B clusters assigned using cluster A centroids mostly show what was in the full dataset clustering. Some of the cereals shift between clusters like in cluster 2 it changes a bit but overall, the groupings are consistent and show medium to high stability for the clusters.
...
cluster_summary <- aggregate(cereal_numeric.df, by = list(Cluster = cereal_clusters$Cluster), FUN = mean)
cluster_summary
